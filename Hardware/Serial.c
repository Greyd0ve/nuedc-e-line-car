/* 项目适配说明：USART1 用作蓝牙/网页调参串口，PA9=TX、PA10=RX。接收中断写入环形缓冲区，BT_proto.c 再解析方括号协议。 */
/*
 * 文件：Hardware/Serial.c
 * 作用：USART1 串口/蓝牙通信。
 *
 * TX=PA9，RX=PA10，默认 9600bps。接收中断把字节放进环形缓冲区，
 * BT_proto.c 再从缓冲区中解析 [] 数据包。
 *
 * 注释版说明：仅增加注释，未改变串口逻辑。
 */
...
void USART1_IRQHandler(void)
{
	if (USART_GetITStatus(USART1, USART_IT_RXNE) == SET)
	{
		...
		uint16_t next = (Serial_RxW + 1) % SERIAL_RX_BUF_SIZE;
		if (next != Serial_RxR) {           // 满了就丢
